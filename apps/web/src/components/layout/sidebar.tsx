...
export function Sidebar() {
    ...
    (e: TouchEvent) => {
      ...
      if (sidebarRef.current) {
        if (mobileOpen && diff > 0) {
          ...
          sidebarRef.current.style.transform = `translateX(${Math.max(-280, translateX)}px)`;
        } else if (!mobileOpen && diff < 0) {
          const translateX = Math.min(280, Math.abs(diff));
          sidebarRef.current.style.transform = `translateX(${280 - translateX}px)`;
        }
      }
    },
    [mobileOpen],
  ...
  const handleTouchEnd = useCallback(() => {
    ...
    if (mobileOpen && diff > threshold) {
      setMobileOpen(false);
    } else if (!mobileOpen && diff < -threshold) {
      setMobileOpen(true);
    }
          ...
          {!collapsed ? (
            <>
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary/20 to-cyan-500/10 border border-primary/20 flex items-center justify-center">
                  <svg
                    viewBox="0 0 24 24"
                    ...
                    return (
                      ...
                      >
                        ...
                        {collapsed && item.showBadge && unreadCount > 0 && (
                          <span className="absolute top-0.5 left-0.5 w-2 h-2 bg-danger rounded-full border-2 border-surface" />
                        )}
                      </Link>
                    );
                  })}
